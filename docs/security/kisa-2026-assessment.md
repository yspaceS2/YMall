...
| 범위 | 1차 판정 | 내용 |
| --- | --- | --- |
| D-01~D-09 계정 관리 | 배포 후 점검 | 운영 전용 계정, 최소 권한, 비밀번호 복잡도·재사용·로그인 실패 정책은 실제 PostgreSQL 역할 설정으로 확인한다. 애플리케이션 컨테이너는 비루트 사용자로 실행된다. |
| D-10 원격 접속 제한 | 양호 | 운영 Compose에서 PostgreSQL 호스트 포트를 노출하지 않고 내부 Docker 네트워크만 사용한다. |
| D-11, D-14, D-18, D-20, D-21 접근·권한 | 배포 후 점검 | 스키마·시스템 테이블·GRANT 권한은 운영 DB 역할 생성 절차에서 검증해야 한다. |
...
| D-26 감사 로그 | 보완 필요 | 애플리케이션 관리자 행위 감사는 있으나 PostgreSQL 접속·DDL·권한 변경 감사 정책은 별도로 구성되어 있지 않다. |
