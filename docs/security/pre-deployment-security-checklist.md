...
- [x] 업로드 파일의 서명과 실제 디코딩, 손상된 PNG, 변조된 WebP 컨테이너 및 경로 이동 파일명을 확인
...
- `LocalFileStorageServiceTest`: 손상 이미지, WebP 컨테이너, 경로 이동 파일명 차단
...
- `compose.prod.yaml`: Redis 호스트 포트 제거, PostgreSQL·Kafka 내부 네트워크 전용, Frontend loopback 기본 바인딩

## 배포 후 점검

- [ ] 운영 HTTPS/TLS와 인증서 구성
