...
| `PARTIAL_CANCELED` | 결제사 취소 내역과 서버의 환불 이력을 대조하고 부분 환불 상태를 반영한다. 환불된 수량은 이후 판매자 배송·정산 대상에서 제외한다. |
...
부분 환불과 잔여 금액 환불은 누적 환불액이 원결제 금액을 넘지 않도록 검증한다. 같은 멱등키의 재요청은 기존 결과를 반환하며, 판매자가 이미 배송 처리한 수량과 충돌하는 환불은 상태 전이 규칙에 따라 거부한다.
