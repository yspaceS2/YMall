...
> 이 문서는 2026-07-24 기준 후보 선정 기록이다. 2026-08-10 실데이터 재검증에서
> 0.6B 모델의 긴 입력 JSON 안정성 한계가 확인되어 현재 서비스 모델은
> `Qwen3-4B-GGUF Q4_K_M`으로 변경했다. 최신 운영 검증은
> [AI 리뷰 요약 기능 검증](review-summary-feature-validation.md)을 따른다.
...
| 캐시 상태 로딩 | 1.3442초 | 4.8420초 |
| 샘플 생성 p50 | 0.4173초 | 2.7212초 |
| 샘플 생성 p95 | 0.6193초 | 2.7343초 |
| 엄격 JSON 객체 준수율 | 0% | 0% |
| 복구 가능 JSON 객체율 | 0% | 100% |
| 필수 스키마 준수율 | 0% | 100% |
...
Qwen은 모든 샘플에서 Markdown 코드 펜스로 감싼 JSON 객체와 세 섹션을 만들었다.
따라서 응답 원문 자체의 엄격 JSON 준수율은 0%이고, 코드 펜스를 제거했을 때의 복구
가능 JSON 객체율과 필수 스키마 준수율은 100%다. 운영 연동에서는 복구 파싱에
의존하기보다 모델 출력 제약과 응답 검증을 강화해야 한다. ROUGE-L이 낮은 이유에는
기준 문구와 다른 자연스러운 표현을 사용한 점도 포함되므로 이 수치만으로 사실성을
판정하지 않는다.
...
2. 모든 평가 샘플의 출력을 JSON 객체로 복구할 수 있고 필수 스키마를 지켰다.
