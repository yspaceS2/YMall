            ...
            <div className="mx-auto w-[calc(100%-40px)] max-w-125 py-14 min-[601px]:w-[calc(100%-48px)] min-[601px]:py-20">
                ...
                <form className="grid gap-6" onSubmit={submit}>
                    <label className="grid gap-2 text-xs font-bold text-muted">
                        <span>이메일</span>
                        <input className={inputClassName} type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" placeholder="you@example.com" maxLength={255} required />
                    ...
                    <button className="mt-2 h-13.5 border border-ink bg-ink font-extrabold text-white transition-colors hover:bg-[#333] disabled:cursor-default disabled:opacity-60" disabled={saving}>
