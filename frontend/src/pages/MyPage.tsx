...
const memberSections: MemberSection[] = [
    'dashboard',
    'profile',
    'email',
    'social',
...
const memberSectionPaths: Record<string, MemberSection> = {
    '/mypage': 'dashboard',
    '/mypage/profile': 'profile',
    '/mypage/email': 'email',
    '/mypage/social': 'social',
    '/mypage/wishlist': 'wishlist',
    '/mypage/addresses': 'addresses',
    '/mypage/seller-application': 'seller-application',
}
...
export function MyPage() {
    ...
    const activeSection = memberSectionPaths[location.pathname] ?? 'dashboard'

    useEffect(() => {
        if (location.pathname !== '/mypage') return
        const querySection = new URLSearchParams(location.search).get('section')
        const legacySection = querySection ?? location.hash.slice(1)
        if (
            legacySection
            && legacySection !== 'dashboard'
            && memberSections.includes(legacySection as MemberSection)
        ) {
            navigate(`/mypage/${legacySection}`, { replace: true })
        }
    ...
    async function handleProfileSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            showToast(
                error instanceof ApiError ? error.message : '회원 정보를 저장하지 못했습니다.',
                'error',
            )
        } finally {
    ...
    const quickLinks = [
        { label: '프로필', description: '회원 정보와 보안 설정', href: '/mypage/profile', icon: UserRound },
        { label: '찜', description: '관심 상품과 판매 상태', href: '/mypage/wishlist', icon: Heart },
        { label: '배송지', description: '받는 주소 추가·수정', href: '/mypage/addresses', icon: MapPin },
        { label: '주문', description: '주문 및 배송 현황', href: '/mypage/orders', icon: ClipboardList },
        { label: '리뷰', description: '구매 상품 리뷰 관리', href: '/mypage/orders', icon: MessageSquareText },
        { label: '알림', description: '주문 상태 알림 확인', href: '/mypage/notifications', icon: Bell },
    ]
        ...
        <section className="mx-auto w-[calc(100%-40px)] max-w-240 py-14 min-[601px]:w-[calc(100%-48px)] min-[601px]:py-20">
            ...
            <nav className="mt-10 grid grid-cols-2 gap-3 min-[701px]:grid-cols-3 min-[901px]:grid-cols-6" aria-label="마이페이지 바로가기">
                {quickLinks.map(({ label, description, href, icon: Icon }) => {
                    ...
                    return <Link className={className} to={href} key={label}>{content}</Link>
                })}
            </nav>
            )}
            ...
            <div className="mt-8 grid gap-8 min-[901px]:grid-cols-2">
            ...
            <div className="mt-8 border border-line bg-surface p-6 min-[601px]:p-8">
            ...
            {activeSection === 'seller-application' && <SellerApplicationPanel />}
        </section>
    )
}
