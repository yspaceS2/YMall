...
export function ProductCard({ product }: { product: ProductSummary }) {
    ...
    return (
        <article className="group relative min-w-0">
            <Link className="relative block aspect-[.82] overflow-hidden bg-subtle" to={`/products/${product.productId}`}>
                {product.thumbnailUrl ? (
                    ...
                    <div className="grid size-full place-items-center bg-linear-to-br from-subtle to-line font-serif text-lg font-bold tracking-[.2em] text-muted">YMALL</div>
                )}
            ...
            <Link className="block pt-4" to={`/products/${product.productId}`}>
                ...
                <div className="flex items-baseline gap-2">
                    {product.discountPercentage > 0 && <strong className="text-accent">{product.discountPercentage}%</strong>}
                    <b className="text-sm">{formatPrice(discountedPrice)}</b>
                    {product.discountPercentage > 0 && <del className="text-[11px] text-muted">{formatPrice(product.price)}</del>}
                </div>
