...
- 모든 대상 상품은 상품군의 실제 구매 기준과 고유 특징을 반영한 서로 다른 리뷰 문장을 사용한다.
- 스크립트를 다시 실행하면 기존 fixture 리뷰의 평점과 본문도 최신 상품별 문장으로 갱신된다.
- 장점과 단점이 모호하게 뒤섞이지 않도록 평가 방향을 분명히 작성해 요약 분류 오류를 줄인다.
